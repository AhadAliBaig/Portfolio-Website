  ...
  return (
    <section id="contact" className="pt-16 md:pt-20 pb-10 md:pb-12 px-6 max-w-7xl mx-auto">
      <div className="flex items-center gap-4 mb-12">
        ...
        <h2 className="font-mono text-sm uppercase tracking-[0.3em] text-aluminum">
          Contact Me
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        <div>
          <h3 className="font-headline font-extrabold text-4xl md:text-5xl text-aluminum mb-6 uppercase tracking-tight">
            Let&apos;s Connect
          </h3>
          <p className="text-aluminum/60 text-lg leading-relaxed mb-10">
          ...
          <div className="mt-12 p-6 bg-charcoal-low border border-outline-v/15 rounded-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="font-mono text-[10px] uppercase tracking-widest text-aluminum">
        ...
        <div className="bg-charcoal-low border border-outline-v/15 p-8 md:p-12 rounded-sm">
          <form onSubmit={handleSubmit} className="space-y-6">
            {state.succeeded && (
              <p className="font-mono text-[10px] text-cyan-glow text-center uppercase tracking-widest">
                Message sent successfully! I&apos;ll get back to you soon.
              </p>
            )}
            {state.errors && Object.keys(state.errors).length > 0 && (
              <p className="font-mono text-[10px] text-red-500 text-center uppercase tracking-widest">
            ...
            <div>
              <label
                htmlFor="email"
                className="block font-mono text-[10px] uppercase tracking-widest text-aluminum/40 mb-2"
              >
                Your Email
              </label>
              <input
                ...
                className="w-full bg-obsidian border border-outline-v/20 p-4 font-mono text-sm text-aluminum focus:outline-none focus:border-cyan-glow transition-colors"
              />
              <ValidationError
                prefix="Email"
                field="email"
            ...
            <div>
              ...
              <textarea
                ...
                disabled={state.submitting}
                className="w-full bg-obsidian border border-outline-v/20 p-4 font-mono text-sm text-aluminum focus:outline-none focus:border-cyan-glow transition-colors resize-none"
              />
              <ValidationError
                prefix="Message"
                field="message"
            ...
            <button
              type="submit"
              disabled={state.submitting}
              className="w-full flex items-center justify-center gap-3 bg-aluminum text-obsidian-dark font-mono text-xs font-bold py-4 uppercase tracking-widest hover:bg-cyan-glow transition-all active:scale-95 disabled:opacity-50"
            >
              {state.submitting ? (
                "Sending..."
              ) : (
...
function ContactInfo({
  ...
  return (
    ...
    >
      ...
      <div>
        <div className="font-mono text-[8px] uppercase tracking-widest text-aluminum/30">
          {label}
        </div>
        <div className="font-mono text-xs text-aluminum group-hover:text-cyan-glow transition-colors">
          {value}
        </div>
      </div>
    </a>
